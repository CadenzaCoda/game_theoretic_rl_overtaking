...
class PPOSelfPlayTrainer:
    ...
    def collect_rollout(self, max_steps: int = 2048) -> Tuple[np.ndarray, ...]:
        ...
        ego_wins, oppo_wins, draws, truncations, ego_losses, oppo_losses = 0, 0, 0, 0, 0, 0
        ego_avg_speed, oppo_avg_speed = 0, 0
        ...
        # logger.info(f"Logging from {"Ego" if ego_vehicle else "Oppo"} vehicle")
        ...
        for _ in trange(max_steps, desc='Collect'):
            ...
            ego_avg_speed += info['ego']['avg_eps_speed']
            oppo_avg_speed += info['oppo']['avg_eps_speed']
            
            if terminated['ego'] or truncated['ego'] or terminated['oppo'] or truncated['oppo']:  # Reset on either condition
                if info['ego']['success']:
                    ego_wins += 1
                if info['ego']['failure']:
                    ego_losses += 1
        ...
        if episode_rewards:
            ...
            self.writer.add_scalar('rollout/ego_wins', ego_wins, self.episode_count)
            self.writer.add_scalar('rollout/oppo_wins', oppo_wins, self.episode_count)
            self.writer.add_scalar('rollout/ego_losses', ego_losses, self.episode_count)
            self.writer.add_scalar('rollout/oppo_losses', oppo_losses, self.episode_count)
            self.writer.add_scalar('rollout/draws', draws, self.episode_count)
            self.writer.add_scalar('rollout/truncations', truncations, self.episode_count)
            self.writer.add_scalar('rollout/ego_avg_speed', ego_avg_speed / max_steps, self.episode_count)
            self.writer.add_scalar('rollout/oppo_avg_speed', oppo_avg_speed / max_steps, self.episode_count)
        logger.info(f"EW: {ego_wins}, OW: {oppo_wins}, EL: {ego_losses}, OL: {oppo_losses}, D: {draws}, G: {truncations}")
        logger.info(f"EV: {ego_avg_speed / max_steps:.2f}, OV: {oppo_avg_speed / max_steps:.2f}")
