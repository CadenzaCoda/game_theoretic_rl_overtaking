...
from mpclab_common.pytypes import VehicleState, ParametricPose, \
...
class BarcEnv(gym.Env):
    ...
    def _is_new_lap(self):
        ...
        return do_intersect(np.array([self.sim_state.x.x, self.sim_state.x.y]),
                            ...
                            np.array([0, -self.track_obj.half_width])) and self.sim_state.p.s < self.last_state.p.s
