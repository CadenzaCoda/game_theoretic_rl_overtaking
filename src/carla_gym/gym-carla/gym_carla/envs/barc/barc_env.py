...
class MultiBarcEnv(gym.Env):
    ...
    def _get_reward(self) -> float:
        ...
        if np.linalg.norm(np.array([self.sim_state[0].x.x, self.sim_state[0].x.y]) - np.array(
                [self.sim_state[1].x.x, self.sim_state[1].x.y])) < 0.3:
            return -100.0
...
class BarcEnvRace(gym.Env):
    ...
    def step(self, action: ActType) -> Tuple[ObsType, float, bool, bool, dict]:
        opponent_action, _ = self.opponent.step(self.sim_state[1], lap_no=self.lap_no[1], terminated=self._is_new_lap()[1])
