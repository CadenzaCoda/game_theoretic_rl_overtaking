...
class CasadiDynamicsModel(AbstractModel):
    ...
    def step(self, vehicle_state: VehicleState, inplace=True) -> VehicleState:
        '''
        steps noise-free model forward one time step (self.dt) using numerical integration
        '''
        if not inplace:
            vehicle_state = deepcopy(vehicle_state)
...
class CasadiKinematicCLBicycle_flat(CasadiDynamicsModel):
    '''
    ...
    def __init__(self, t0: float, model_config: KinematicBicycleConfig = KinematicBicycleConfig(), track=None):
        ...
        self.sym_ax     = ca.SX.sym('ax')
        self.sym_del     = ca.SX.sym('delta')
        
        # turning angle
        L = self.L_f + self.L_r
        self.sym_beta = ca.atan2(ca.tan(self.sym_del) * self.L_f, L)

        self.fun_beta=ca.Function("beta", [self.sym_del], [self.sym_beta])
        ...
        # time derivatives
        self.sym_dv     = self.sym_ax #- 0.1*self.sym_v/self.m
        self.sym_depsi      = self.sym_psidot - self.sym_c * self.sym_v * ca.cos(self.sym_epsi + self.sym_beta) / (1 - self.sym_xtran * self.sym_c)
        self.sym_ds         = self.sym_v * ca.cos(self.sym_epsi + self.sym_beta) / (1 - self.sym_xtran * self.sym_c)
        self.sym_dxtran     = self.sym_v * ca.sin(self.sym_epsi + self.sym_beta)

        self.sym_q = ca.vertcat(self.sym_s, self.sym_xtran, self.sym_epsi, self.sym_v)
        self.sym_u = ca.vertcat(self.sym_ax, self.sym_del)
        self.sym_dq = ca.vertcat(self.sym_ds, self.sym_dxtran, self.sym_depsi, self.sym_dv)

        self.R = 2
        self.sym_Y = ca.SX.sym('Y_lo', self.R*self.n_u)

        # Rewriting the relation F_x(Y)=x as Fx_1(Y)=Fx_2(Y,x) to remove singularities

        # self.Fx_1 = ca.vertcat(self.sym_Y[:self.n_u],  self.sym_Y[-1]-self.sym_Y[-1-self.n_u],
        #                     (self.sym_Y[-1]-self.sym_Y[-1-self.n_u])**2+(1-self.sym_c*self.sym_q[1])**2*(self.sym_Y[-self.n_u]-self.sym_Y[0])**2)
        # self.Fx_2 = ca.vertcat(self.sym_q[:self.n_u],
        #                      (1-self.sym_c*self.sym_q[1])*(self.sym_Y[-self.n_u]-self.sym_Y[0])*ca.tan(self.sym_q[2]),
        #                      self.sym_q[-1]**2*self.dt**2)
        self.Fx_1 = ca.vertcat(self.sym_Y[:self.n_u],  self.sym_Y[-1]-self.sym_Y[-1-self.n_u])
        self.Fx_2 = ca.vertcat(self.sym_q[:self.n_u],
                             (1-self.sym_c*self.sym_q[1])*(self.sym_Y[-self.n_u]-self.sym_Y[0])*ca.tan(self.sym_q[2]))

        self.fun_Fx_eq = ca.Function('Fx_eq', [self.sym_q, self.sym_Y], [self.Fx_1-self.Fx_2], ['x','Y'], ['Fx'])
    ...
    def state2qu(self, state: VehicleState) -> Tuple[np.ndarray, np.ndarray]:
        q = np.array([state.p.s, state.p.x_tran, state.p.e_psi, np.sqrt(state.v.v_long**2+state.v.v_tran**2)])
        u = np.array([state.u.u_a, state.u.u_steer])
        return q, u
    
    def dynamic2kinematic(self, state: VehicleState) -> VehicleState:
        state_out=VehicleState()
        state_out.t=copy.copy(state.t)
        state_out.p.s=copy.copy(state.p.s)
        state_out.p.e_psi=copy.copy(state.p.e_psi)
        state_out.p.x_tran=copy.copy(state.p.x_tran)
        state_out.v.v_long=np.sqrt(state.v.v_long**2+state.v.v_tran**2)
        state_out.u.u_steer=copy.copy(state.u.u_steer)
        state_out.u.u_a=copy.copy(state.u.u_a)

        return state_out
    
    def error_dyn2kin(self, kin_state: VehicleState, dyn_state: VehicleState) -> np.ndarray:
        dyn2kin_state=self.dynamic2kinematic(dyn_state)
        track_L=self.track.track_length
        s_err=dyn2kin_state.p.s-kin_state.p.s
        s_errs=np.array([s_err,s_err-track_L,s_err+track_L])
        s_err=s_errs[np.argmin(np.abs(s_errs))]
        return np.array([s_err, dyn2kin_state.p.x_tran-kin_state.p.x_tran, dyn2kin_state.p.e_psi-kin_state.p.e_psi, dyn2kin_state.v.v_long-kin_state.v.v_long])
    
    def q2y(self, q: np.ndarray) -> np.ndarray:
        if len(q.shape)==1:
            return q[:self.n_u]
        else:
            return q[:, :self.n_u]
    ...
    def q2state(self, state: VehicleState, q: np.ndarray, u: np.ndarray):
        state.v.v_long  = q[3]
        state.p.e_psi   = q[2]
        state.p.s       = q[0]
        state.p.x_tran  = q[1]
    ...
    def qu2state(self, state: VehicleState, q: np.ndarray = None, u: np.ndarray = None):
        if q is not None:
            ...
            state.p.x_tran  = q[1]
            if u is not None:
                state.w.w_psi = q[3] / self.L_r * np.sin(np.arctan(np.tan(u[1]) * self.L_f / (self.L_f + self.L_r)))
    ...
    def Y2prediction(self, prediction: VehiclePrediction, Y: np.ndarray = None):
        if prediction is None:
            prediction = VehiclePrediction()

        def Y2state(Y_l):
            states=np.zeros((Y_l.shape[0], self.n_q))
            for i in range(Y_l.shape[0]):
                y=Y_l[i,:]
                states[i,0:self.n_u]=y[:self.n_u]
                states[i,-2]=np.arctan((y[-1]-y[-1-self.n_u])/(1-self.get_curvature(y[0])*y[1])/(y[-self.n_u]-y[0]))
                states[i,-1]=np.sqrt((y[-1]-y[-1-self.n_u])**2+(1-self.get_curvature(y[0])*y[1])**2*(y[-self.n_u]-y[0])**2)/self.dt
            return states
       
        if Y is not None:
            q = Y2state(Y)
            prediction.s        = array.array('d', Y[:, 0])
            prediction.x_tran   = array.array('d', Y[:, 1])
            prediction.e_psi    = array.array('d', q[:, 2])
            prediction.v_long   = array.array('d', q[:, 3])
        return prediction
...
class CasadiCLPointMass(CasadiDynamicsModel):
    '''
    Frenet frame of reference point mass

    Body frame velocities and track frame positions
    '''
    def __init__(self, t0: float, model_config: PointMassConfig = PointMassConfig(), track=None):
        super().__init__(t0, model_config, track=track)
        ...
        self.sym_xtran  = ca.SX.sym('xtran')
        self.sym_Fx     = ca.SX.sym('Fx')
        self.sym_Fy     = ca.SX.sym('Fy')

        self.sym_c = self.get_curvature(self.sym_s)

        # time derivatives
        self.sym_dv = (self.sym_Fx - self.c_da*self.sym_v)/self.m
        self.sym_depsi = self.sym_Fy/(self.m*self.sym_v) - self.sym_c * (self.sym_v * ca.cos(self.sym_epsi)) / (1 - self.sym_xtran * self.sym_c)
        self.sym_ds = self.sym_v * ca.cos(self.sym_epsi) / (1 - self.sym_xtran * self.sym_c)
        self.sym_dxtran = self.sym_v * ca.sin(self.sym_epsi)

        # state and state derivative functions
        self.sym_q = ca.vertcat(self.sym_v, self.sym_epsi, self.sym_s, self.sym_xtran)
        self.sym_u = ca.vertcat(self.sym_Fx, self.sym_Fy)
        self.sym_dq = ca.vertcat(self.sym_dv, self.sym_depsi, self.sym_ds, self.sym_dxtran)

        self.f_a = ca.Function('f_a', [self.sym_q, self.sym_u], [self.sym_Fx/self.m, self.sym_Fy/self.m, 0], self.options('f_a'))
        self.f_ang_a = ca.Function('f_ang_a', [self.sym_q, self.sym_u], [0, 0, 0], self.options('f_ang_a'))
...
class CasadiPointMassCombined(CasadiDynamicsModel):
    '''
    Frenet frame of reference point mass
...
try:
    from mpclab_common.models.model_types import VehicleConfig3D

    from barc3d.pytypes import VehicleState as VehicleState3D
    from barc3d.dynamics.vehicle_models import VehicleModel as VehicleModel3D
    from barc3d.dynamics.vehicle_models import DynamicsModelConfig, TireLayout, TireModel, LongitudinalInput, WeightDistribution
    from barc3d.utils.mpc_utils import MPCResults
...
if barc3d_is_available:
    class CasadiDynamicBicycle3D(CasadiDynamicsModel):
        def __init__(self, t0: float,
                            vehicle_config: VehicleConfig3D,
                            track,
                            print_method=print):

            super().__init__(t0, vehicle_config)

            self.vehicle_config = vehicle_config
            self.vehicle_config.model_config.dt = self.dt

            self.t0 = t0
            self.print_method = print_method

            if isinstance(track, RadiusArclengthTrack):
                self.surf = track.get_barc3d_surface()
            elif isinstance(track, BaseSurface):
                self.surf = track
            else:
                self.print_method('Track type not supported')
            L = self.surf.s_max()

            class VehicleModel(VehicleModel3D):
                def __init__(self, vehicle_config, surf):
                    if vehicle_config.tire_layout == 'bicycle':
                        tire_layout = TireLayout.BICYCLE
                    elif vehicle_config.tire_layout == 'two_track':
                        tire_layout = TireLayout.TWO_TRACK

                    if vehicle_config.tire_model == 'kinematic':
                        tire_model = TireModel.KINEMATIC
                    elif vehicle_config.tire_model == 'pacejka':
                        tire_model = TireModel.PACEJKA
                    elif vehicle_config.tire_model == 'linear':
                        tire_model = TireModel.LINEAR

                    if vehicle_config.input_model == 'accel':
                        input_model = LongitudinalInput.ACCEL
                    elif vehicle_config.input_model == 'slip':
                        input_model = LongitudinalInput.SLIP
                    elif vehicle_config.input_model == 'accel_tire':
                        input_model = LongitudinalInput.ACCEL_TIRE
                    elif vehicle_config.input_model == 'accel_rear_tire':
                        input_model = LongitudinalInput.ACCEL_REAR_TIRE

                    if vehicle_config.weight_distribution == 'planar':
                        weight_distribution = WeightDistribution.PLANAR
                    elif vehicle_config.weight_distribution == 'quasiplanar':
                        weight_distribution = WeightDistribution.QUASIPLANAR
                    elif vehicle_config.weight_distribution == 'dynamic_ode':
                        weight_distribution = WeightDistribution.DYNAMIC_ODE
                    elif vehicle_config.weight_distribution == 'dynamic_soft_ode':
                        weight_distribution = WeightDistribution.DYNAMIC_SOFT_ODE
                    elif vehicle_config.weight_distribution == 'dynamic_dae':
                        weight_distribution = WeightDistribution.DYNAMIC_DAE

                    config = DynamicsModelConfig()
                    config.tire_layout = tire_layout
                    config.tire_model = tire_model
                    config.planar = vehicle_config.planar
                    config.input_model = input_model
                    config.weight_distribution = weight_distribution
                    super().__init__(vehicle_config.model_config, surf, config)

            self.dynamics_model = VehicleModel(self.vehicle_config, self.surf)

            self.n_q = self.dynamics_model.f_zdot.size_in(0)[0]
            self.n_u = self.dynamics_model.f_zdot.size_in(1)[0]

            self.sym_q = ca.SX.sym('q', self.n_q)
            self.sym_u = ca.SX.sym('u', self.n_u)
            _q = ca.vertcat(ca.fmod(self.sym_q[0], L), self.sym_q[1:])
            self.sym_dq = self.dynamics_model.f_zdot(_q, self.sym_u)
        ...
        def state_to_state3d(self, state: VehicleState, state_3d: VehicleState3D):
            ...
            state_3d.tfl        = state.tfl
            state_3d.tfr        = state.tfr
            state_3d.trl        = state.trl
            state_3d.trr        = state.trr
        ...
        def state3d_to_state(self, state_3d: VehicleState3D, state: VehicleState):
            ...
            state.tfl           = state_3d.tfl
            state.tfr           = state_3d.tfr
            state.trl           = state_3d.trl
            state.trr           = state_3d.trr

        def step(self, vehicle_state):
            if isinstance(vehicle_state, VehicleState):
                vehicle_state_3d = VehicleState3D()
                self.state_to_state3d(vehicle_state, vehicle_state_3d)
            else:
                vehicle_state_3d = vehicle_state
            self.surf.frenet_to_global(vehicle_state_3d)
            self.dynamics_model.step(vehicle_state_3d)
            if isinstance(vehicle_state, VehicleState):
                self.state3d_to_state(vehicle_state_3d, vehicle_state)
                q = vehicle_state_3d.q.to_vec()
                y, p, r = Rotation.from_quat(q).as_euler('ZYX')
                vehicle_state.e.phi = r
                vehicle_state.e.theta = p
                vehicle_state.e.psi = y

        def state2qu(self, state: VehicleState) -> Tuple[np.ndarray, np.ndarray]:
            state_3d= VehicleState3D()
            self.state_to_state3d(state, state_3d)
            q, u = self.dynamics_model.state2zu(state_3d)
            return np.array(q), np.array(u)

        def state2q(self, state: VehicleState) -> np.ndarray:
            state_3d= VehicleState3D()
            self.state_to_state3d(state, state_3d)
            q, _ = self.dynamics_model.state2zu(state_3d)
            return np.array(q)

        def input2u(self, input: VehicleActuation) -> np.ndarray:
            state = VehicleState3D()
            state.u.a = input.u_a
            state.u.y = input.u_steer
            _, u = self.dynamics_model.state2zu(state)
            return np.array(u)

        def q2state(self, state: VehicleState, q: np.ndarray):
            state_3d = VehicleState3D()
            self.dynamics_model.zu2state(state_3d, q, np.zeros(self.n_u))
            self.state3d_to_state(state_3d, state)
            return

        def u2input(self, state: VehicleState, u: np.ndarray):
            state_3d = VehicleState3D()
            self.dynamics_model.u2state(state_3d, u)
            self.state3d_to_state(state_3d, state)
            return

        def qu2state(self, state: VehicleState, q: np.ndarray, u: np.ndarray):
            if q is None:
                q = np.zeros(self.n_q)
            if u is None:
                u = np.zeros(self.n_u)
            state_3d = VehicleState3D()
            self.dynamics_model.zu2state(state_3d, q, u)
            self.state3d_to_state(state_3d, state)
            return
        ...
        def qu2prediction(self, prediction: VehiclePrediction, q: np.ndarray = None, u: np.ndarray = None):
            ...
            if q is not None:
                prediction.v_long   = array.array('d', q[:, 3])
                prediction.v_tran   = array.array('d', q[:, 4])
                prediction.psidot   = array.array('d', q[:, 5])
                prediction.e_psi    = array.array('d', q[:, 2])
                prediction.s        = array.array('d', q[:, 0])
                prediction.x_tran   = array.array('d', q[:, 1])
            if u is not None:
        ...
        def qu2mpcresults(self, prediction: MPCResults, q: np.ndarray = None, u: np.ndarray = None, t0=0):
            if q is not None:
                N = q.shape[0]
            elif u is not None:
                N = u.shape[0]
            prediction = MPCResults(states=[VehicleState3D() for _ in range(N)])

            if q is not None:
                for i, _q in enumerate(q):
                    prediction.states[i].p.s    = _q[0]
                    prediction.states[i].p.y    = _q[1]
                    prediction.states[i].p.ths  = _q[2]
                    prediction.states[i].v.v1   = _q[3]
                    prediction.states[i].v.v2   = _q[4]
                    prediction.states[i].w.w3   = _q[5]
                T = t0 + self.dt*np.arange(0, q.shape[0])
                t_sym = ca.SX.sym('t')
                interps = []
                for j in range(q.shape[1]):
                    interps.append(ca.pw_lin(t_sym, T, q[:,j]))
                z_interp = ca.Function('z', [t_sym], [ca.vertcat(*interps)])
                prediction.z_interp = lambda t: np.array(z_interp(t)).squeeze()
                prediction.time = T[-1]-T[0]
            if u is not None:
                for i, _u in enumerate(u):
                    prediction.states[i].u.a    = _u[0]
                    prediction.states[i].u.y    = _u[1]
                T = t0 + self.dt*np.arange(0, u.shape[0])
                t_sym = ca.SX.sym('t')
                interps = []
                for j in range(u.shape[1]):
                    interps.append(ca.pw_lin(t_sym, T, u[:,j]))
                u_interp = ca.Function('u', [t_sym], [ca.vertcat(*interps)])
                prediction.u_interp = lambda t: np.array(u_interp(t)).squeeze()
            return prediction
...
def get_dynamics_model(t_start: float, model_config: DynamicsConfig, track=None) -> CasadiDynamicsModel:
    ...
    elif model_config.model_name == 'dynamic_bicycle_3d':
        return CasadiDynamicBicycle3D(t_start, model_config, track=track)
    ...
    elif model_config.model_name == 'kinematic_bicycle_cl_flat':
        return CasadiKinematicCLBicycle_flat(t_start, model_config, track=track)
...
def get_dynamics_model_params(model_name: str):
    if '3d' in model_name:
        return VehicleConfig3D()
    elif 'dynamic_bicycle' in model_name:
...
if __name__ == '__main__':
    ...
    config = DynamicBicycleConfig(model_name='dynamic_bicycle_cl', 
                                  track_name=track_name, 
                                  dt=0.1, 
                                  noise=True,
                                  noise_cov=4*np.eye(6), 
                                  code_gen=True, 
                                  jit=True, 
                                  opt_flag='O3', 
                                  verbose=True)
    dynamics = CasadiDynamicCLBicycle(0.0, config)

    q0 = np.array([2, 3, 2, -0.1, 0.2, 0.03])
    u0 = np.array([1, -0.3])

    state = VehicleState(t=0.0)
    dynamics.qu2state(state, q0, u0)
